...
We show that $\gamma$ is monotonically increasing in $\beta_1^g$ for $\pi = 1/2$. First, we compute the derivative of (\ref{gamma_alt2_pi_half}) with respect to $\beta^g_1$. The quotient rule implies that
\begin{multline}\label{d_gamma_d_beta}
D_1 \gamma(\beta^g_1, c, 1/2) \\ = \frac{D_1[2g(\beta^g_1, c, 1/2)] 4h(\beta^g_1, c, 1/2) - 2g(\beta^g_1, c, 1/2)D_1[4h(\beta^g_1, c, 1/2)]}{[4h(\beta^g_1, c, 1/2)]^2}.
\end{multline}
First,
...
To show that $\gamma$ is monotonically increasing in $\beta^g_1$, it is sufficient to show that (\ref{d_gamma_d_beta}) is nonnegative, i.e.
\begin{equation}\label{thm:gamma_monotonic_1}
...
\end{equation}
The product $\textcolor{violet}{D_1[2g(\beta^g_1, c, 1/2)]}\textcolor{red}{4h(\beta^g_1, c, 1/2)}$ is
\begin{multline}\label{g_prime_times_h}
\textcolor{violet}{D_1[2g(\beta^g_1, c, 1/2)]}\textcolor{red}{4h(\beta^g_1, c, 1/2)} \\ = \textcolor{violet}{f(\beta^g_0 + \beta^g_1 - c)}\textcolor{red}{\left[ \Phi(\beta_0^g - c ) + \Phi(\beta^g_0 + \beta^g_1 - c)\right]\left[\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g) \right]}.
\end{multline} 
Next, the product $\textcolor{blue}{2g(\beta^g_1, c, 1/2)}\textcolor{teal}{D_1[4h(\beta^g_1, c, 1/2)]}$ is
\begin{multline}\label{g_times_h_prime}
\textcolor{blue}{2g(\beta^g_1, c, 1/2)}\textcolor{teal}{D_1[4h(\beta^g_1, c, 1/2)]} \\ = \textcolor{teal}{f(\beta^g_0 + \beta^g_1 - c)}\textcolor{blue}{\left[\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)\right]}\textcolor{teal}{\left[\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g) \right]} \\ - \textcolor{teal}{f(\beta^g_0 + \beta^g_1 - c)}\textcolor{teal}{\left[\Phi(\beta_0^g - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \right]} \textcolor{blue}{\left[\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)\right]}.
\end{multline}
Divide both sides by $f(\beta^g_0 - \beta^g_1 - c) > 0$. Then, add
$$\textcolor{teal}{\left[\Phi(\beta_0^g - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \right]}\textcolor{blue}{\left[\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)\right]}.$$ The left-hand side becomes
\begin{multline*}
\textcolor{red}{\left[ \Phi(\beta_0^g - c ) + \Phi(\beta^g_0 + \beta^g_1 - c)\right]\left[\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g) \right]} \\ +\textcolor{teal}{\left[\Phi(\beta_0^g - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \right]}\textcolor{blue}{\left[\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)\right]} \\ = \left[\Phi(\beta^g_0 - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \right] \\ \cdot \left[\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g) + \Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c) \right].
\end{multline*}
Meanwhile, the right-hand side becomes
$$\left[\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)\right]\left[\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g) \right].$$
Therefore, we want to demonstrate that
\begin{multline}\label{basic_ineq}
\left[\Phi(\beta^g_0 - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \right] \\ \cdot \left[\textcolor{brown}{\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)} + \textcolor{olive}{\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g)} \right] \\ \geq \left[\textcolor{brown}{\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c)}\right]\left[\textcolor{olive}{\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g)}\right].
\end{multline}
(We changed the color scheme to highlight like terms.)
We take cases on the sign on $\beta^g_1$ to prove this inequality.
\\ \\ \noindent
\textbf{Case 1}: $\beta^1_g < 0$. Then $ \beta^g_1 + (\beta^g - c) < (\beta^g_0 - c),$ implying $\Phi(\beta^g_0 + \beta^g_1 - c) < \Phi(\beta^g_0 - c),$ or $[\Phi(\beta^g_0 + \beta^g_1 - c) - \Phi(\beta^g_0 - c)] < 0.$ Moreover, the quantity $[\Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g)]$ is positive. Thus, the right-hand side of (\ref{basic_ineq}) is negative.

Turning our attention of the left-hand side, we have that
\begin{multline*}
\Phi(\beta^g_0 + \beta^g_1 - c) + \Phi( c - \beta^g_0 - \beta^g_1) = \int_{-\infty}^{ \beta^g_0 + \beta^g_1 - c} f + \int_{-\infty}^{c - \beta^g_0 - \beta^g_1} f \\ = \int_{-\infty}^{ \beta^g_0 + \beta^g_1 - c} f  + \int_{\beta^g_0 + \beta^g_1 - c}^\infty f = 1.
\end{multline*}
Additionally, $\Phi(\beta^g_0 - c) < 1,$ and $ \Phi(c - \beta^g_0) > 0$. Combining these expressions, we find that
$$ \left[\Phi(\beta_0^g + \beta_1^g - c) - \Phi(\beta_0^g - c) + \Phi(c - \beta^g_0) + \Phi(c - \beta_0^g - \beta_1^g) \right] > 0. $$ Because $\left[\Phi(\beta^g_0 - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \right] > 0,$ it follows that the entire left-hand side of (\ref{basic_ineq}) is positive.
\\ \\ \noindent
\textbf{Case 2}: $\beta^1_g \geq 0.$  We show that the first term on the LHS of (\ref{basic_ineq}) is greater than the first term on the RHS, and likewise that the second term on the LHS is greater than the second term on the RHS. First,
$$ \Phi(\beta^g_0 - c) \geq - \Phi(\beta^g_0 - c),$$ implying
$$ \Phi(\beta^g_0 - c) + \Phi(\beta^g_0 + \beta^g_1 - c) \geq \Phi(\beta^g_0 - \beta^g_1 - c) - \Phi(\beta^g_0 - c).$$
Second, $\beta^g_1 \geq 0$ implies that $\beta^g_1 + \beta^g_0 - c \geq \beta^g_0 - c,$ and so $$\Phi(\beta^g_1 + \beta^g_0 - c) - \Phi(\beta^g_0 - c) \geq 0.$$ Finally, 
$$ \Phi(\beta^g_1 + \beta^g_0 - c) - \Phi(\beta^g_0 - c) + \Phi(c - \beta^g_0) + \Phi(c - \beta^g_0 - \beta^g_1) \geq \Phi(c - \beta^g_0) + \Phi(c - \beta^g_0 - \beta^g_1).$$
Therefore, the inequality (\ref{basic_ineq}) holds.

\subsubsection{Attenuation bias}

For given $(\beta^g_0, \beta^g_1)$, 
