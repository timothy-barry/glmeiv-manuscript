...
$$ \hat{\beta}^m_1 = \frac{ \frac{1}{n} \sum_{i=1}^n ( \hat{p}_i - \overline{\hat{p}_i})(m_i - \overline{m})}{ \frac{1}{n} \sum_{i=1}^n (\hat{p}_i - \overline{\hat{p}})}.$$ By WLLN,
...
\begin{itemize}
\item[1.] \textbf{Expectation of $m_i$}: $\E[m_i] = \beta^m_0 + \beta^m_1\pi.$
\item[2.] \textbf{Expectation of $\hat{p}_i$}: Let $f$ denote the $N(0,1)$ density. Then \begin{multline*}
...
$$ \omega = \int_{c - \beta^g_1 - \beta^g_0} f = 1 - \Phi(c - \beta^g_1 - \beta^g_0).$$
\item[3.] \textbf{Expectation of $\hat{p}_i p_i$}: 
...
\item[4.] \textbf{Expectation of $\hat{p}_i m_i$}:
\begin{multline*}
...
\end{multline*}
\item[5.] \textbf{Variance of $\hat{p}_i$}: Because $\hat{p}_i$ is binary, we have that $\V[\hat{p}_i] = \E[\hat{p}_i]\left(1 - \E[\hat{p}_i]\right) .$
\item[6.] \textbf{Covariance of $\hat{p}_i, m_i$}:
\begin{multline*}
...
Suppose $\pi = 1/2$. We rewrite the attenuation fraction $\gamma$ in a way that makes it more amenable to theoretical analysis.
