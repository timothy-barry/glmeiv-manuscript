...
\item 
\begin{proposition} For all $\beta^g_0, \beta^g_1,$ and $c \in \R,$ the incurs strict attenuation bias (Figure \ref{thresholding_theoretical}a, all panels).
\end{proposition}
\end{itemize}
...
To shed light on possible limitations of the large threshold selection strategy, we derive an exact bias-variance decomposition for the thresholding estimator. We consider a slightly simpler, no-intercept version of (\ref{theoretical_model}):
...
\end{cases}
\end{equation}
The thresholding estimator $\hat{\beta}_m$ is given by
...
$$\sqrt{n}\left(\hat{\beta}_m - l \right) \xrightarrow{d} N\left(0, v\right).$$
\end{proposition}
The bias decreases and the variance increases as the threshold tends to infinity, consistent with the intuition that a large threshold yields a low misclassification rate at the cost of a small ``effective sample size.'' The best strategy for maximizing estimation accuracy (as quantified by mean squared error) is to select a threshold that induces moderate bias. A downside of this approach, however, is that constructing valid confidence intervals becomes more challenging.
