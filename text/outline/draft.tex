...
We briefly introduce notation that we will use throughout. For $k \in \{0,1\}$, let $\tilde{x}_i(k) := [1, k, z_i]^T$ denote the value of $\tilde{x}_i$ that results from setting $p_i$ to $k$. Next, let $l^m_i(k) := \langle \tilde{x}_i(k), \beta_m \rangle + o^m_i$ and $\eta^m_i(k) := h_m(l^m_i(k))$, and let the corresponding gRNA quantities $l^g_i(k)$ and $\eta_i^g(k)$ be defined analogously. Also, let the design matrix $\tilde{X} \in \R^{n \times d}$ be defined by
...
\vdots & \vdots & \vdots \\
1 & p_n & z_n
\end{bmatrix}.$$ For $k \in \{0,1\}$, let $\tilde{X}(k) \in \R^{n \times d}$ be the matrix that results from setting $p_i$ to $k$ for all $i \in \{1, \dots, n\}$ in $\tilde{X}$.
...
\mathcal{L}(\theta; m_i, g_i, p_i) = \sum_{i=1}^n \log\left( \pi^{p_i}(1-\pi)^{1-p_i} \right) \\ + \sum_{i=1}^n \log\left( f_m(m_i; \eta^m_i)\right) + \sum_{i=1}^n \log\left( f_g(g_i; \eta_i^g) \right). \end{multline}
