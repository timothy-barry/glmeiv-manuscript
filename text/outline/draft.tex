...
% We anticipate three practical uses of the GLM-EIV in single-cell CRISPR screen analysis: first, determining the extent of background contamination, which will inform subsequent analysis decisions. Second, if the amount of background contamination is sufficiently high, thresholding will fail, and so GLM-EIV should be used to get a good answer. On the other hand, if the amount of background contamination is low, we would use the two methods in conjunction: first, fit GLM-EIV to some subset of the data (e.g., the positive control pairs) to get the approximate Bayes-optimal decision boundary; and second, using this decision boundary in the thresholding method. As we have shown, the threshold exerts a substantial impact on the results. In this way, GLM-EIV becomes a threshold selection procedure, enabling statistically rigorous and scientifically reproducible threshold selection procedure, eliminating the aribrariness of selecting a threshold by, for example, eye.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{../../figures/dec_tree/dec_tree_image}
\end{figure}
