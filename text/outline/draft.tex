...
The most promising biotechnology for solving this challenge are high MOI single-cell CRISPR screens. High MOI single-cell CRISPR screens combine CRISPR interference (CRISPRi) -- a version of CRISPR that \textit{represses} a targeted region of the genome, leaving the primary DNA sequence intact  -- with single-cell sequencing. The experimental protocol is as follows. First, the scientist develops a library of several hundred to several thousand CRISPRi perturbations, each designed to target a candidate enhancer for repression. The scientist then cultures tens or hundreds of thousands of cells and delivers the CRISPRi perturbations to the cells. The perturbations assort into the cells randomly, with each cell receiving on average 10-40 distinct perturbations. Conversely, a given perturbation enters about 0.1-2\% of cells. 

After waiting several days for CRISPRi to take effect, the scientist profiles each cell's transcriptome (i.e., its gene expressions) and the set of perturbations that it received. Finally, the scientist conducts perturbation-to-gene association analyses. Figure \ref{analysis_challenges}a depicts this process schematically, with colored bars (blue, red, and purple) representing distinct perturbations. For a given perturbation (e.g., the perturbation represented in blue), the scientist partitions the cells into two groups: those that received the perturbation (top) and those that did not (bottom). Next, for a given gene, the scientist runs a differential expression analysis across the two groups of cells, producing an estimate for the extent to which the gene's expression has changed in response to the perturbation. If the estimated change in expression is large, the scientist can conclude that the enhancer \textit{targeted} by the perturbation exerts a strong regulatory effect on the gene. This procedure is repeated for a large set of preselected perturbation-gene pairs. % If the differential expression analyses produce $p$-values, the $p$-values are subjected to a multiple hypothesis testing correction procedure.

\subsection{Analysis challenges}
High MOI single-cell CRISPR screens present several statistical challenges. We take a parametric approach in this work, and so we highlight challenges that are especially relevant to modeling the data generating process. Throughout, we consider a single perturbation-gene pair. First, the ``treatment'' variable -- i.e., the presence or absence of a perturbation -- is unobservable. Perturbed cells transcribe molecules called  \textit{guide RNAs} (or \textit{gRNAs}) that serve as indirect signals of perturbation presence. We can leverage the gRNA data to infer whether a given cell has been perturbed. Using these imputed perturbation assignments, we can estimate the effect of the perturbation on gene expression (Figure \ref{analysis_challenges}b). Second, ``technical factors'' -- sources of variation that are experimental rather than biological in origin -- impact both gene expression and gRNA expression (Figure \ref{analysis_challenges}b).  We must account for these technical factors in constructing models for gene and gRNA expression. Third, the gene and gRNA data are sparse, discrete counts. Therefore, classical statistical approaches that assume Gaussianity or homoscedasticity are inapplicable. Finally, and most subtly, sequenced gRNAs sometimes are mapped to unperturbed cells. This phenomenon, which we call the ``background read'' phenomenon, results from errors in the sequencing process \cite{Replogle2020}. The marginal distribution of the gRNA count data is best conceptualized as a mixture model (Figure \ref{analysis_challenges}c; Gaussian distributions used for illustration purposes only). Unperturbed and perturbed cells both exhibit nonzero gRNA count distributions, but the gRNA count distribution for perturbed cells is greater. Figure \ref{analysis_challenges}d shows example data for four (of possibly tens or hundreds of thousands of) cells. The analysis objective is to leverage the observed data -- i.e., the gene expressions and gRNA counts -- to estimate the effect of the (latent) perturbation on gene expression, accounting for the technical factors.

In this work we analyze two large-scale, high MOI, single-cell CRISPRi screen datasets published by Gasperini et al.\ and Xie et al. Gasperini (resp., Xie) targeted approximately 6,000 (resp., 500) candidate enhancers in a population of approximately 200,000 (resp., 100,000) cells. Gasperini additionally designed 381 positive control, gene-targeting perturbations and 50 non-targeting, negative control perturbations to assess method sensitivity and specificity.

\section{Thresholding method}

The method that Gasperini et al.\ used in their original analysis is the so-called ``thresholding method,'' or ``thresholded regression.'' In this section we study the thresholding method from empirical and theoretical perspectives, uncovering several basic limitations of the approach. Let $n \in \N$ be the number of cells assayed in the experiment. Consider a single perturbation and a single gene. For cell $i \in \{1, \dots, n\}$, let $m_i \in \N$ be the number of gene transcripts sequenced; let $g_i \in \N$ be the number of gRNA transcripts sequenced; let $l^m_i \in \N$ be the number of gene transcripts sequenced across \textit{all} genes (the library size); and finally, let $z_i \in \R^{d-1}$ be the cell-specific technical factors (e.g., sequencing batch, percent mitochondrial reads, etc.) The letters ``m,'' ``g'', and ``l'' stand for ``mRNA,'' ``gRNA,'' and ``library,'' respectively. The thresholding method is defined as follows:
\begin{itemize}
...
We hypothesized, however, that the reduction in bias conferred by selecting a large threshold comes with the cost of increasing the variance of the estimator. To investigate, we compared $p$-values and confidence intervals produced by threshold = 5 and threshold = 20 for the target of inference $\beta^m_1$. We found that threshold = 5 yielded smaller (i.e., more significant) $p$-values and narrower confidence intervals than did threshold = 20 (Figure \ref{thresholding_empirical}c-d). We concluded that the threshold controls a bias-variance tradeoff: as the threshold increases, bias decreases and variance increases.
...
Next, we study the thresholding method from a theoretical perspective, recovering the attenuation bias and bias-variance tradeoff effects established in the previous section, as well as several other interesting phenomena.  We work in an idealized Gaussian setting. Suppose that we observe gRNA and gene expression data $\{(g_1, m_1), \dots, (g_n, m_n)\}$ on $n \in \N$ cells from the following model:
