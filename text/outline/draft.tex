...
We implement several statistical accelerations (that likely are of independent utility) to bring the cost of GLM-EIV down to within about an order of magnitude of the thresholding method. Finally, we develop a Docker-containerized application to deploy GLM-EIV at-scale across tens or hundreds of nodes on clouds (e.g., Microsoft Azure, Amazon Web Services) and high-performance clusters. Leveraging this application, we apply GLM-EIV to analyze two recent, large-scale, high multiplicity-of-infection single-cell CRISPR screen datasets, yielding new biological and statistical insights.
...
We conducted a simulation study to compare the empirical performance of GLM-EIV and the thresholding method. We generated data on $n = 150,000$ cells from the GLM-EIV model (Section \ref{sec:model}), setting the the target of inference $\beta^m_1$ to $\log(0.25)$ and the probability of perturbation $\pi$ to $0.02$. Note that $\beta^m_1 = \log(0.25)$ denotes a reduction in gene expression by a factor of $4$; this is a fairly large effect size on the order of a positive control pair. We included ``sequencing batch'' (Bernoulli-distributed) as a covariate and sequencing depth (Poisson-distributed) as an offset. We varied the log-fold change in gRNA expression $\beta^g_1$ over a grid defined on $[\log(1), \log(4)];$ $\beta^g_1$ controls problem difficulty, with higher values corresponding to easier problem settings. Finally, we generated the gene expression and gRNA count data using two response distributions: Poisson and negative binomial (with fixed size parameter $\theta = 20$). For each parameter setting (defined by a $\beta^g_1$-distribution pair), we synthesized $n_\textrm{sim} = 500$ datasets. Section \ref{sec:extra_sims} presents additional simulation results on Gaussian response distributions.

We applied three methods to the simulated data: ``standard'' GLM-EIV (Algorithm \ref{algo:em_full}), accelerated GLM-EIV (Algorithm \ref{algo:pilot_estimates}), and thresholded regression. We applied the methods to the negative binomial data twice: once taking the size parameter $\theta$ as known, and once taking $\theta$ as unknown. In the latter case we used the \texttt{glm.nb} function from the \texttt{MASS} package to estimate $\theta$ \cite{Ripley2013}. We display the results of the simulation study in Figure \ref{main_text_sim}. Columns correspond to distributions (i.e., Poisson, negative binomial with known $\theta$, and negative binomial with unknown $\theta$), and rows correspond to performance metrics (i.e., bias, mean-squared error, CI coverage rate (nominal rate $95\%$), CI width, and method execution time). The parameter $\beta^g_1$ is plotted on the $x$-axis. The methods are depicted in different colors (GLM-EIV = red, accelerated GLM-EIV = blue, and thresholding = purple; GLM-EIV masked by accelerated GLM-EIV in several panels).

First, we observed that GLM-EIV uniformly dominated thresholded regression on statistical performance metrics. GLM-EIV exhibited lower bias (row 1) and mean-squared error (row 2) than thresholded regression; additionally, GLM-EIV had superior confidence interval coverage (row 3) despite having produced generally narrower confidence intervals (row 4). Intuitively, GLM-EIV outperformed the thresholding method because (i) GLM-EIV leveraged information from \textit{both} modalities (rather than the gRNA modality alone) to assign perturbation identities to cells, and (ii) GLM-EIV produced soft rather than hard assignments. Additionally, we observed that accelerated GLM-EIV exhibited identical performance to (standard) GLM-EIV on statistical performance metrics (rows 1-4) despite having a substantially reduced computational cost (bottom row). In fact, the execution time of accelerated GLM-EIV was approximately within an order of magnitude of that of the thresholding method (bottom row).

% Interestingly, thresholded regression exhibited better CI coverage for theta est. rather than theta known (row 3). Estimating theta leads to slight \textit{inflation} bias (i.e., \textit{overestimating} the effect size), whereas thresholdeding leads to slight \textit{attenuation} bias (as demonstrated previously). These two phenomena cancel, causing the thresholding method to exhibit better CI coverage when estimating theta. Our focus on this work is \textit{not} on negative binomial size parameter estimation; GLM-EIV and thresholded regression are (in principle) compatible with \textit{any} negative binomial size parameter estimation procedure, including those based on more sophisticated techniques, such as regularization. An interesting future direction would be to assess the performance of the methods under different size parameter estimation approaches.

\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{../../figures/main_text_sim/plot}
	\caption{\textbf{Simulation study}. Columns correspond to distributions (Poisson, NB with known $\theta$, NB with estimated $\theta$), and rows correspond to metrics (bias, MSE, coverage, CI width, and time). Methods are shown in different colors; GLM-EIV (red) is masked by accelerated GLM-EIV (blue) in several panels. GLM-EIV demonstrated superior statistical performance to the thresholding method across all metrics (rows 1-4). Accelerated GLM-EIV had substantially lower computational cost than standard GLM-EIV (bottom row) despite having exhibited identical statistical performance (rows 1-4).}
	\label{main_text_sim}
\end{figure}
...
\section{Additional simulation results}\label{sec:extra_sims}
\begin{figure}
	\centering
