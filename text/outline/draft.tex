...
\blue{In this section we study the thresholding method from empirical and theoretical perspectives, uncovering several limitations of the approach. Gasperini and Xie both imputed perturbation identities onto cells via thresholding, but they carried out subsequent differential expression analyses in different ways: Gasperini ran negative binomial regressions, while Xie conducted nonparametric tests of independence. These two approaches pose similar challenges, but we investigate the regression-based version of the thresholding method here, as it relates most closely to GLM-EIV.}
...
$$\mu_i = \exp \left( \beta^m_0 + \beta^m_1 \hat{p}_i + \gamma^T_m z_i \right) l_i^m.$$ We see that $\exp \left( \beta^m_0 + \beta^m_1 \hat{p}_i + \gamma^T_m z_i \right)$ is the \textit{fraction} of transcripts in the cell produced by the gene under consideration.}
