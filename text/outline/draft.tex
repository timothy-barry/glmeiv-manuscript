...
Leveraging our computational infrastructure (Section \ref{sec:computing}), we applied GLM-EIV to analyze the entire Gasperini and Xie datasets (Figure \ref{fig:real_data}). Our theoretical analyses (Section \ref{sec:thresholding_theory}) and simulation studies (Section \ref{sec:simulation}) revealed that the Bayes-optimal decision boundary is a good choice for the threshold when the gRNA count distribution is well separated. Therefore, under the assumption that the effect of the perturbation on gRNA expression is similar across pairs, we used the fitted GLM-EIV models to approximate the Bayes-optimal decision boundary for both datasets, yielding a boundary of $3$ for Gasperini and $7$ for Xie. We then applied the thresholding method to analyze both datasets, setting the threshold to the Bayes-optimal decision boundary estimated using GLM-EIV.

% We compared GLM-EIV to thresholded regression on the real data and surprisingly found that the two methods produced similar results. Figures \ref{fig:real_data}a and \ref{fig:real_data}b show the estimate of the target of inference $\exp(\beta^m_1)$ 

To better understand this result, we examined the estimated effect of the perturbation on gRNA expression, $\exp(\hat{\beta}_1^g)$; this parameter plays an important role in determining problem difficulty. The value of $\exp(\hat{\beta}_1^g)$ was greater than expected: the 95\% CI for this parameter averaged across pairs was $[XX,XX]$ and $[300, 316]$ on the Gasperini and Xie data, respectively. We concluded that both datasets lay in an ``easy'' region of the parameter space, making thresholding a tenable strategy (provided the threshold is selected well).

To illuminate the settings in which GLM-EIV and the thresholding method diverge, we artificially increased the problem difficulty by generating partially-synthetic datasets. To this end, we sampled gRNA counts from a realistic model, holding fixed the raw gene expressions, library sizes, covariates, and fitted perturbation probabilities. We defined a normalized parameter called ``excess background contamination'' that controlled the problem difficulty. When excess background contamination was equal to zero, we sampled gRNA counts directly from the fitted model. As excess background contamination increased, we sampled gRNA counts from a slightly modified version of the fitted model in which the mean gRNA expression of \textit{unperturbed} cells was increased while the mean gRNA expression of \textit{perturbed} cells was held constant. Finally, when excess background contamination was equal to $1$ (the maximum value), we set the mean gRNA expression of unperturbed cells equal to that of perturbed cells and sampled from the resulting model.

We generated partially-synthetic data for each of the 322 positive control pairs in the Gasperini dataset, varying excess background contamination over the interval $[0,0.4].$ We applied GLM-EIV and the thresholding method to analyze these data. We present results on two representative pairs (the pair containing gene \textit{LRIF1} and the pair containing gene \textit{NDUFA2}) in Figures \ref{fig:real_data}c-d. First, we observed that the estimate produced by the methods on the raw data (depicted as a horizontal black line) coincided almost exactly with the estimate produced by the methods on the partially-synthetic data generated by setting excess background contamination to zero (trend present across all genes; average relative difference $0.003$). This indicates that the fitted model accurately tracked the gRNA count distribution. Next, we observed that, as excess background contamination increased, the thresholding method exhibited severe attenuation bias, whereas GLM-EIV remained roughly stable.

We conducted a similar analysis on the entire set of positive control pairs. First, for each pair, we computed the ``relative estimate change'' (REC), defined as the relative difference between the estimate at some level of excess contamination and zero excess contamination (Figure \ref{fig:real_data}d). % We plotted median 

\begin{figure}
	\centering
	\includegraphics[width=0.9\linewidth]{../../figures/data_analysis/annotated_plot_crop}
	\caption{\textbf{Data analysis}. \textbf{a-b}, Fold change estimates produced by GLM-EIV and thresholded regression on Gasperini (\textbf{a}) and Xie (\textbf{b}) negative control pairs. \textbf{c-d}, Estimates produced by GLM-EIV and thresholded regression on two positive control pairs -- \textit{LRIF1} (\textbf{a}) and \textit{NDUFA2} (\textbf{b}) -- plotted as a function of excess background contamination. Grey bands, 95\% CIs outputted by the methods. \textbf{e-f}, Median relative estimate change (REC; \textbf{e}) and confidence interval coverage rate (\textbf{f}) aggregated across \textit{all} 322 positive control pairs, plotted as a function of excess background contamination. Colored bands, pointwise 95\% CIs for population quantities. Panels (\textbf{c-f}) together illustrate that GLM-EIV was more stable than thresholded regression as background contamination increased.}
	\label{fig:real_data}
\end{figure}
