...
$$\mu_i = \exp \left( \beta^m_0 + \beta^m_1 \hat{p}_i + \gamma^T_m z_i \right) d_i^m.$$ The term $\exp \left( \beta^m_0 + \beta^m_1 \hat{p}_i + \gamma^T_m z_i \right)$ is the \textit{fraction} of all transcripts sequenced in the cell produced by the gene under consideration. The target of inference, $\beta^m_1$, is the log-transformed fold change in gene expression in response to the perturbation, \blue{controlling for} the technical factors. Fold change (i.e., $\exp(\beta^m_1)$) is the ratio of mean gene expression in perturbed cells to unperturbed cells: $\exp(\beta^m_1) = 1$ indicates no change in expression, while $\exp(\beta^m_1) > 1$ and $\exp(\beta^m_1) < 1$ indicate an increase and decrease in expression in response to the perturbation, respectively (accounting for technical factors).
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{../../figures/main_text_sim/plot}
	\caption{}
	\label{main_text_sim}
\end{figure}
