...
\blue{In their original analysis of the data, Gasperini and Xie imputed perturbation identities onto the cells by thresholding the gRNA counts. In this section we study the thresholding method from empirical and theoretical perspectives, uncovering several limitations of the approach. We investigate the specific version of the thresholding method used by Gasperini, which involved a thresholding step and a follow-up negative binomial regression step. (Xie implemented a slightly different and, in our view, less principled version of thresholding, which we do not review here.)}
...
% Setting $\gamma_m^T$ to zero and exponentiating both sides of (\ref{thresh_glm}), we obtain
% $$\mu_i = \left(e^{\beta^m_0 + \beta^m_1\hat{p}_i}\right) l^m_i.$$ The mean expression of unperturbed cells $\mu_i(0)$ is
% $$\mu_i(0) = e^{\beta^m_0} l_i^m,$$ and the mean expression in perturbed cells is
% $$ \mu_i(1) = e^{\beta^m_1} e^{\beta^m_0} l^m_i.$$ Therefore,
% $$ \beta^m_1 = \log\left(\frac{\mu_i(1)}{\mu_i(0)}\right).$$
% We use library size $l_i^m$
